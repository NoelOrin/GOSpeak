...
## [0.4.0](https://github.com/NoelOrin/GOSpeak/compare/v0.3.1...0.4.0) (2026-09-04)

### Features

* auth: 会话惰性续期前端链路与登录限流分桶
* web: 邀请落地页与注册页改为独立路由
* oauth: 回调 postMessage payload 携带 expires_in
* auth: login/register/reset/refresh/oauth 响应下发 expires_in
* auth: access/refresh TTL 支持 JWT_ACCESS_TTL/JWT_REFRESH_TTL 配置
...
* domain: 启动回填存量域系统角色
* web: 根治路由切换闪烁的三处残留根因
* message: 实时广播 DTO 携带 author 富字段
* web: 移除管理页入场动画与路由多余 default export
* domain: 创建语音服务器时播种系统角色
* cluster: 内嵌总线跳过选主与写面 fence

### Documentation

* 按代码现状全量修订 AGENTS.md
* 会话主动续期（expires_in + 懒续期）实现计划
* 会话过期下发与主动续期（expires_in + 懒续期）设计规格
* 补充 v0.3.0 changelog（游客访问权限体系 + 登录权限重构）

### Refactoring

* web: 图标全量迁移 lucide-solid

### Styles

* 清零剩余 Biome 警告（未使用参数 / 可选链）
* 清理遗留 Biome 警告（ForgotPasswordModal / kvStore.test）
* web: 清理 Biome 警告（未使用 catch 绑定 / 非空断言 / forEach 返回值）

### Tests

* message: 覆盖广播 DTO 的 author 富字段
