...
func (h *Hub) OnRoomCreate(s socketio.Conn, data string) {
	...
	// 持久化到数据库
	if h.roomStore != nil {
		dbRoom := &model.Room{Name: req.Room}
		if err := h.roomStore.Create(dbRoom); err != nil {
			log.Printf("[Signal] failed to persist room %s: %v", req.Room, err)
		} else {
			roomInfo.ID = dbRoom.ID
			roomInfo.UUID = dbRoom.UUID
		}
	}
