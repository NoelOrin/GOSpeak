...
| jwt.go | JWT 生成/解析；每个 Token 携带 ULID 作为 JTI；签名密钥由 `authstate.GetSigningKey()` / `GetAllSigningKeys()` 提供（支持密钥轮换与历史密钥）；access 15m、refresh 7d、bot 可永久 |
...
| permcode/ | 权限码常量（`user:read` 等），供 middleware 与路由使用。权限码分布在 `permcode.go`（通用权限）与 `domain_permcode.go`（Domain 专属权限）|
