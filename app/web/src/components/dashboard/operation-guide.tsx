...
const GUIDE_ITEMS = [
	{ title: "创建或加入房间", text: "在左侧频道列表双击房间进入，或用首页快捷入口先进入频道页再操作。" },
	{ title: "音量与静音", text: "底部用户栏和房间内控件可调整输入输出音量，也可以快速切换本地静音状态。" },
	{ title: "退出与切换", text: "进入房间后可在右上角离开，返回频道列表后可以继续切换到其他房间。" },
	{ title: "管理入口", text: "管理员可以直接进入权限、禁言和 SFU 配置页面处理日常运营操作。" },
] as const;
					...
					<For each={GUIDE_ITEMS}>
						{(item) => (
							<div class="rounded-lg border border-base-300/70 bg-base-200/40 px-4 py-4">
								...
								<div class="mt-2 text-sm leading-6 text-base-content/70">{item.text}</div>
							</div>
						)}
					</For>
				</div>
