	...
	return (
		<section class="rounded-lg border border-base-300 bg-base-100 p-5 shadow-sm">
			<div class="mb-4">
				...
				<p class="text-sm text-base-content/60">按在线人数展示当前最活跃的房间</p>
			</div>
					...
					{(room, index) => (
						<div class="rounded-lg border border-base-300/70 px-4 py-3">
							<div class="flex items-center justify-between gap-4">
								<div>
									<div class="text-sm text-base-content/50">TOP {index() + 1}</div>
									<div class="font-medium">{room.name}</div>
								</div>
								<div class="text-right">
									<div class="text-lg font-semibold">{room.count}</div>
									<div class="text-xs text-base-content/50">容量 {room.limit}</div>
								</div>
							</div>
							<progress
